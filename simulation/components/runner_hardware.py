from i2c_com.i2c_util import I2CDevice
...
class ComponentHardware(ComponentSim):
    def __init__(self, address: int) -> None:
        ...
        self.log.info(
            f"Established {self.__class__.__name__}({hex(address)}) at I2C address {hex(address)}"
        )
    ...
    def request(self, request_code: int, data: int) -> int:
        ...
        if data is not None:
            self._i2c_device.send([request_code, data])
        else:
            self._i2c_device.send(request_code)
        response = self._i2c_device.read(1)[0]
        return response
